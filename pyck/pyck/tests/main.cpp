...
#define __EXCP(CODE) do { try { CODE; } catch( const std::exception &e ) { std::cerr << e.what() << std::endl; } } while(false)
    ...
    int main(int argc, char *argv[])
    {
        try
        {
            ...
            RunTimeClock clk;
            {
                const uint64_t ini = RunTimeClock::Ticks();
                for(size_t i=0;i<10;++i)
                {
                    const uint64_t t = RunTimeClock::Ticks();
                    std::cerr << "clk ticks=" << std::hex << t << std::dec << std::endl;
            ...
            // Testing C_Matrix
            ////////////////////////////////////////////////////////////////////////
            std::cerr << "-- Testing C_Matrix" << std::endl;
            C_Matrix<double> M(2,3);
