...
Every time a matrix $\J_n$ is compute then $\JW_n=\J_n\W$ is computed, as well as a representation of $\A_n = \left(\JW_n\tJ_n\right)^{-1}$.
An upgrade of positions is
$$
	\q_{k+1} = \q_{k} - \tJW_{k} \A_{k} \C\left(\q_{k}\right).
$$
A legalization of the velocities is
$$
	\dq_{k+1} = \dq_{k} - \tJW_{k+1} \A_{k+1} \J_{k+1} \dq_{k}
$$
