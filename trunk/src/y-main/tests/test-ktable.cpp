...
YOCTO_UNIT_TEST_IMPL(ktable)
{
	...
	for( size_t i=0; i <= 3000; i += (1+alea_less_than<size_t>(30)) )
	{
		...
		{
			dummy *dn = new(tab.cache.query()) dummy( alea_less_than<size_t>(kmax) );
			tab.insert(dn->a,dn);
		}
		...
		for( size_t j=0; j < tab.nodes; ++j )
		{
			...
			const size_t k  = alea_less_than<size_t>(kmax);
