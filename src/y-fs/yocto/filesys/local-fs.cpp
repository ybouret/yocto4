	...
	namespace filesys
	{
		...
		void local_fs:: create_dir(  const string &dirname, const bool allow_already_exists )
		{
			...
			if( (attr != INVALID_FILE_ATTRIBUTES) && ( (attr & FILE_ATTRIBUTE_DIRECTORY) != 0 ) ) {
				...
				{
					throw windows::exception( ERROR_ALREADY_EXISTS, "create_dir(%s)", &dirname[0] );
				}
				else
					return;
			}
			...
			{
				throw windows::exception( ::GetLastError(), "create_dir(%s)", &dirname[0] );
			}
#endif
		...
		void local_fs:: remove_dir(  const string &dirname   )
		{
			...
			if(! ::RemoveDirectory( &dirname[0] ) ) {
				throw windows::exception( ::GetLastError(), "remove_dir(%s)", &dirname[0] );
			}
#endif
		}
		...
		void local_fs:: remove_file( const string &filename )
		{
			...
			{
				throw windows::exception( ::GetLastError(), "::DeleteFile(%s)", &filename[0] );
			}
#endif
		}
