...
YOCTO_UNIT_TEST_IMPL(chunk)
{
	...
	double ell = 0;
	size_t naq = 0;
	for( size_t block_size=1; block_size <= 40; block_size += (1+alea_less_than<size_t>(4)) )
	{
		std::cerr << "block_size=" << block_size << std::endl;
		for( size_t num_blocks = chunk::num_blocks_min; num_blocks <= chunk::num_blocks_max; num_blocks += (1+alea_less_than<size_t>(4)) )
		{
