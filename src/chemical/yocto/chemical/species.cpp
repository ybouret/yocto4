...
namespace yocto
{
    ...
    namespace  chemical
    {
        ...
        species::species(const string &sp_name, const int sp_charge, const groups &sp_formula ) :
        name(sp_name),
        charge(sp_charge),
        formula(sp_formula)
        {
        ...
        namespace
        {
            ...
            //
            //==================================================================
            class fullBuilder : public builder
            {
            public:
                string name;
                int    charge;
                ...
                inline fullBuilder(const xnode *molecule) :
                name(),
                charge(0),
                formula()
                {
                ...
                inline virtual ~fullBuilder() throw()
                {
                }
                ...
                inline void onStoi( const xnode *stoi , groups &grp)
                {
                    assert(stoi);
                    assert("stoi"==stoi->label);
                    assert(1==stoi->size()||2==stoi->size());
            ...
            private:
                YOCTO_DISABLE_COPY_AND_ASSIGN(fullBuilder);
            };
        }
        ...
        string species:: nameOf( const xnode *molecule )
        {
            nameBuilder nb(molecule);
            return nb.name;
        }

        species * species:: create(const xnode *molecule)
        {
            fullBuilder fb(molecule);
            return new species(fb.name,fb.charge,fb.formula);
        }
