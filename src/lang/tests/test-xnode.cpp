...
YOCTO_UNIT_TEST_IMPL(xnode)
{
    ...
    auto_ptr<syntax::xnode> pTerm( syntax::xnode::create(termID, new lexeme(termID,1), syntax::is_regular) );
    *static_cast<token *>(pTerm->lxm()) = "hello";
    
    const string otherID = "other";
    auto_ptr<syntax::xnode> pOther( syntax::xnode::create(otherID,syntax::is_regular) );
